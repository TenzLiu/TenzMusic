...
public class SearchSongListAdapter extends BaseQuickAdapter<SearchSongResponse.InfoBean> {
    ...
    public SearchSongListAdapter(Context context, int layoutResID, List<SearchSongResponse.InfoBean> data, Option option) {
        super(context, layoutResID, data);
        this.mOption = option;
    }

    @Override
    public void convert(BaseViewHolder holder, SearchSongResponse.InfoBean item) {
        ...
        tv_song_name.setText(item.getSongname());
        tv_singer.setText(item.getSingername().replace("<em>","").replace("</em>",""));
